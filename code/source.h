...
typedef struct font_data
{
    ...
    s32 atlas_width;
    s32 atlas_height;
    u32 atlas_sampler_binding;
    VkSampler atlas_sampler;
    VkImage atlas_image;
    VkImageView atlas_imageview;
    VkDeviceMemory atlas_memory;
    u32 *atlas_pixels;
    u32 atlas_pixels_datasize;
    // ---
} font_data;
...
string *RetrieveEnum(char *filename, char *enum_name)
{
    ...
    {
        ODS("Wrong enum name!\n");
        ODS("You were searching for this: %s", enum_name);
        exit(0);
    }
...
render_init triangle_render_init;
...
void QuadsWrite(renderstate *rs, quad_uv *quads, u32 quad_count)
{
...
void VertexesWrite(renderstate *rs, vertex *vertexes, u32 vertex_count)
{
    u32 write_size = sizeof(vertex) * vertex_count;
    ...
    if(rs->geometry->used > rs->geometry->size)
    {
        ODS("Vertex geometry pool's been blown out!");
        exit(0);
    }
...
u32 *GenerateIndexes_overlay(renderstate *rs)
{
    u32 n = rs->quad_count;
...
u32 *GenerateIndexes_triangle(renderstate *rs)
{
    rs->index_count = 3;
    u32 *indexes = (u32 *)malloc(sizeof(u32) * rs->index_count);
...
vertex_uv *QuadGenerate_uv(u32 w, u32 h, r32 cx, r32 cy)
{
    r32 width  = (r32)w;
    r32 height = (r32)h;
...
vertex_uv *QuadGenerate_centered_uv(u32 w, u32 h)
{
    ...
    vertex_uv *result = QuadGenerate_uv(w, h, cx, cy);
    
    return result;
}

vertex *QuadGenerate(u32 w, u32 h, r32 cx, r32 cy)
{
    r32 width  = (r32)w;
    r32 height = (r32)h;
                                     ...
                                     string *str, r32 x_topleft, r32 y_topleft)
{
    ...
    {
        u32 d3dstyle = 1;
        stbtt_GetBakedQuad(fdata[font_index].baked_chardata, fdata[font_index].atlas_width, fdata[font_index].atlas_height, str->text[i]-fdata[font_index].first_char, &x_topleft, &y_topleft, &q[i], d3dstyle);
        i++;
    }
    // ---
                                      ...
                                      r32 x_center, r32 y_center)
{
    ...
    {
        u32 d3dstyle = 1;
        stbtt_GetBakedQuad(fdata[font_index].baked_chardata, fdata[font_index].atlas_width, fdata[font_index].atlas_height, str->text[i]-fdata[font_index].first_char, &xcoord, &ycoord, &q[i], d3dstyle);
        i++;
    }
    // ---
                             ...
                             r32 h_padding, r32 v_padding)
{
    ...
    else { ODS("Whoops. Wrong enum value."); exit(0); }
...
void Font_InitAndBake(renderstate *rs, char *fontfile, r32 font_size, u32 color)
{
    ...
    s32 char_count = 96;
    rs->fd->atlas_width  = 512;
    rs->fd->atlas_height = 512;
    
    u8 *bakedfont_bitmap = (u8 *)malloc(rs->fd->atlas_width * rs->fd->atlas_height);
    rs->fd->baked_chardata = (stbtt_bakedchar *)malloc(sizeof(stbtt_bakedchar) * rs->fd->char_count);
    
    stbtt_BakeFontBitmap(ttf_buffer, 0,
                         font_size,
                         bakedfont_bitmap, rs->fd->atlas_width, rs->fd->atlas_height,
                         rs->fd->first_char, rs->fd->char_count,
                         rs->fd->baked_chardata);
    
    rs->fd->atlas_pixels = STB_WritePixels(bakedfont_bitmap, rs->fd->atlas_width, rs->fd->atlas_height, color);
    rs->fd->atlas_pixels_datasize = rs->fd->atlas_width * rs->fd->atlas_height * 4;
...
void RenderString_va(renderstate *rs, u32 font_index, h_align ha, v_align va, u32 h_pad, u32 v_pad, char *format, va_list args)
{
    ...
    QuadsWrite(rs, quads, str->length);
    // ---
...
void EnumerateGlobalExtensions()
{
    ...
    ODS("\n> Instance-wide extensions: %d", global_propscount);
...
void EnumerateLayerExtensions()
{
    ...
    for(u32 i = 0; i < layer_count; i++)
    {
        ...
        if(ext_count)
        {
            ODS(">> its extensions: %d", ext_count);
...
void EnumerateDeviceExtensions()
{
    ...
    ODS("\n> Device extensions: %d", devext_propcount);
...
void EnumerateDeviceLayers()
{
    ...
    ODS("\n> Device layers: %d", dev_layercount);
...
void CheckSurfaceCapabilities()
{
    ...
    ODS("- current extent: %-4d x %-4d", surface_caps.currentExtent.width,  surface_caps.currentExtent.height);
    ODS("- minimal extent: %-4d x %-4d", surface_caps.minImageExtent.width, surface_caps.minImageExtent.height);
    ODS("- maximal extent: %-4d x %-4d", surface_caps.maxImageExtent.width, surface_caps.maxImageExtent.height);
...
{
    ODS("Possible indexes:\n%.*s", 64,        DecToBin(possibleMemoryIndexes));
    ODS("Looking for these flags:\n%.*s", 64, DecToBin(requiredProperties));
...
void SetupQueue()
{
    ...
    for(u32 i = 0; i < queuefam_propcount; i++)
    {
        ...
        ODS("Minimum gran: %-4d x %-4d", queue_famprops[i].minImageTransferGranularity.width, queue_famprops[i].minImageTransferGranularity.height);
...
VkShaderModule RetrieveShaderModule(char *filepath, VkDevice GPUinterface)
{
    ...
    {
        ODS("Couldn't load shader file %s", filepath);
        exit(1);
    }
...
void CreateRenderpass_overlay(renderstate *rs)
{
    CreateRenderpass(rs, true, true);
}
void CreateRenderpass_geometry(renderstate *rs)
{
    CreateRenderpass(rs, false, false);
}

void CreatePipeline_overlay(render_init *di)
{
    // ---
    VkShaderModule vertex_module   = RetrieveShaderModule(di->vert, vk.device);
    VkShaderModule fragment_module = RetrieveShaderModule(di->frag, vk.device);
    ...
    di->rs->fd->atlas_sampler_binding = binding_index++;
    ...
    sampler_binding.binding            = di->rs->fd->atlas_sampler_binding;
    ...
    dpool_ci.pNext         = NULL;
    dpool_ci.flags         = 0;
    dpool_ci.maxSets       = vk.desc_count;
    ...
    dset_ai.descriptorSetCount = vk.desc_count;
    dset_ai.pSetLayouts        = &data.ds_layout;
    vkAllocateDescriptorSets(vk.device, &dset_ai, &data.dset);
    // ---
    ...
    pipeline_lci.pSetLayouts            = &data.ds_layout;
    pipeline_lci.pushConstantRangeCount = 0;
    pipeline_lci.pPushConstantRanges    = NULL;
    
    vkCreatePipelineLayout(vk.device, &pipeline_lci, NULL, &di->rs->pipeline_layout);
    ...
    pipeline_ci.layout              = di->rs->pipeline_layout;
    pipeline_ci.renderPass          = di->rs->renderpass;
    pipeline_ci.subpass             = 0;
    pipeline_ci.basePipelineHandle  = NULL;
    pipeline_ci.basePipelineIndex   = NULL;
    
    vkCreateGraphicsPipelines(vk.device, NULL, 1, &pipeline_ci, NULL, &di->rs->pipeline);
}
void CreatePipeline_geometry(render_init *di)
{
    // ---
    VkShaderModule vertex_module   = RetrieveShaderModule(di->vert, vk.device);
    VkShaderModule fragment_module = RetrieveShaderModule(di->frag, vk.device);
    ...
    vertex_state.vertexBindingDescriptionCount   = 1;
    vertex_state.pVertexBindingDescriptions      = &bind_desc;
    vertex_state.vertexAttributeDescriptionCount = 1;
    vertex_state.pVertexAttributeDescriptions    = &coord_desc;
    // ---
...
void CreateRenderstate_overlay(render_init *di, font_init *ti)
{
    // --- --- --- Text init --- --- ---
    di->rs->fd = (font_data *)malloc(sizeof(font_data));
    di->rs->fd->first_char = 32;
    di->rs->fd->char_count = 96;
    
    Font_InitAndBake(di->rs, ti->fontfile, ti->font_size, ti->color);
    // ---
    
    
    // --- --- --- Render init --- --- ---
    // --- Renderstate memory
    InitRenderstateMemory(di->rs, MEGABYTE, 240 * KILOBYTE);
    
    // --- Pipeline
    CreateRenderpass_overlay(di->rs);
    CreatePipeline_overlay(di);
}
void CreateRenderstate_geometry(render_init *di)
{
    //// --- --- --- Render init --- --- ---
    // --- Renderstate memory
    InitRenderstateMemory(di->rs, KILOBYTE, KILOBYTE);
    
    // --- Pipeline
    CreateRenderpass_geometry(di->rs);
    CreatePipeline_geometry(di);
}
...
void TakeScreenshot(u32 acquired_index, u32 frame_counter)
{
    ...
    TransitImageLayout(VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                       vk.swapchain_images[acquired_index], &vk);
    ...
    TransitImageLayout(VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR,
                       vk.swapchain_images[data.acquired_index], &vk);
...
void DataPrepare()
{
    ...
    overlay_render_init.vert = "../build/text_vert.spv";
    overlay_render_init.frag = "../build/text_frag.spv";
    ...
    triangle_render_init.rs   = &triangle_rs;
    triangle_render_init.vert = "../build/draw_vert.spv";
    triangle_render_init.frag = "../build/draw_frag.spv";
    CreateRenderstate_geometry(&triangle_render_init);
    ...
    data.staging_buffer = CreateBuffer(overlay_rs.fd->atlas_pixels_datasize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                                       VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                                       vk.device, vk.gpu_memprops, &data.staging_memory);
    
    //// --- --- --- ATLAS PREPARATIONS --- --- ---
    void *staging_mapptr;
    vkMapMemory(vk.device, data.staging_memory, 0, VK_WHOLE_SIZE, 0, &staging_mapptr);
    memcpy(staging_mapptr, overlay_rs.fd->atlas_pixels, overlay_rs.fd->atlas_pixels_datasize);
    vkUnmapMemory(vk.device, data.staging_memory);
    
    CreateSamplerImage(overlay_rs.fd->atlas_width, overlay_rs.fd->atlas_height,
                       &overlay_rs.fd->atlas_image,
                       &overlay_rs.fd->atlas_memory);
    CreateImageView(&overlay_rs.fd->atlas_image, &overlay_rs.fd->atlas_imageview);
    
    TransitImageLayout(VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                       overlay_rs.fd->atlas_image, &vk);
    StageImage(overlay_rs.fd->atlas_width, overlay_rs.fd->atlas_height,
               overlay_rs.fd->atlas_image);
    
    CreateSampler(&overlay_rs.fd->atlas_sampler);
    // ---
    
    
    //// --- --- --- GEOMETRY PREPARATIONS --- --- ---
    // Our triangle, screen coords
    data.sc0 = Vector4(  400.0,  200.0,  0.0,  1.0 );
    data.sc1 = Vector4(  600.0,  600.0,  0.0,  1.0 );
    data.sc2 = Vector4(  800.0,  200.0,  0.0,  1.0 );
    
    
    // Orthographics matrix for overlay renderstate's pipeline
    data.ortho = GenerateOrthographicMatrix(app.window_width, app.window_height);
    ...
    memcpy(matrix_mapptr, data.ortho, matrix_datasize);
    vkUnmapMemory(vk.device, data.matrix_memory);
    // ---
    
    
    // Initialize rotating matrix
    data.rotating_matrix = Matrix4();
}

void Process()
{
    // --- clean indexes
    MemoryReset_indexes(&triangle_rs);
    ...
    r32 dt;
#if RECORD
    dt = data.target_frametime;
#elif !RECORD
    dt = data.dt;
#endif
    
    r32 angle = 72.0f;  // one full rotation per 5 seconds
    r32 angle_dt = angle * dt;
    vector4 unit_y = Vector4(0.0f, 1.0f, 0.0f, 0.0f);
    data.rotation_matrix = GenerateRotationMatrix(angle_dt, unit_y);
    data.rotating_matrix = MultMM(data.rotation_matrix, data.rotating_matrix);
    ...
    // scale down 
    r32 scale = 0.01f;
    vector3 scaling_vector = Vector3(scale);
    matrix4 scaling_matrix = GenerateScalingMatrix(scaling_vector);
    
    // move 
    vector3 translation_vector = Vector3(-6.0f, -4.0f, 0.0f);
    matrix4 translation_matrix = GenerateTranslationMatrix(translation_vector);
    
    // --- to view(camera) space
    matrix4 view_matrix = Matrix4();
    vector3 view_translation_vector = Vector3(0.0f, 0.0f, -10.0f);
    view_matrix = TranslateMatrix(view_matrix, view_translation_vector);
    
    // --- clip space
    matrix4 projection = GenerateProjectiveMatrix(90.0f, 16.0f/9.0f, 0.1f, 1000.0f);
    
    // To correctly rotate geometry around its axis, we first move it to origin. After rotation, move back and project...
    r32 *td0 = MultMV(projection, MultMV(view_matrix, MultMV(data.rotating_matrix, MultMV(translation_matrix, MultMV(scaling_matrix, data.sc0)))));
    r32 *td1 = MultMV(projection, MultMV(view_matrix, MultMV(data.rotating_matrix, MultMV(translation_matrix, MultMV(scaling_matrix, data.sc1)))));
    r32 *td2 = MultMV(projection, MultMV(view_matrix, MultMV(data.rotating_matrix, MultMV(translation_matrix, MultMV(scaling_matrix, data.sc2)))));
    
    
    
    vertex *triangle = (vertex *)malloc(sizeof(vertex) * 3);
    u32 vertexsize = sizeof(r32) * 4;
    
    memcpy(triangle,   td0, sizeof(vertex));
    memcpy(triangle+1, td1, sizeof(vertex));
    memcpy(triangle+2, td2, sizeof(vertex));
    
    // ...and write it.
    VertexesWrite(&triangle_rs, triangle, 3);
    
    void *triangle_mapptr;
    vkMapMemory(vk.device, triangle_rs.vertex_memory, 0, VK_WHOLE_SIZE, 0, &triangle_mapptr);
    memcpy(triangle_mapptr, triangle_rs.geometry->start, triangle_rs.geometry->used);
    vkUnmapMemory(vk.device, triangle_rs.vertex_memory);
    
    
    // --- indexes
    // generate
    u32 *triangle_indexes = GenerateIndexes_triangle(&triangle_rs);
    IndexesWrite(&triangle_rs, triangle_indexes);
    
    // write
    void *geometry_index_mapptr;
    vkMapMemory(vk.device, triangle_rs.index_memory, 0, VK_WHOLE_SIZE, 0, &geometry_index_mapptr);
    memcpy(geometry_index_mapptr, triangle_rs.indexes->start, triangle_rs.indexes->used);
    vkUnmapMemory(vk.device, triangle_rs.index_memory);
    ...
    RenderString(&overlay_rs, font_index, left, top, h_pad, v_pad, "Frame #%d", data.frame_counter);
    
    RenderString(&overlay_rs, font_index, left, top, h_pad, v_pad + v_advance, "Warmup time: %6.3f ms", data.time_warmup);
    
    r64 running = TimerElapsedFrom_us(data.time_start);
    string *running_str = TimerString(running);
    RenderString(&overlay_rs, font_index, left, top, h_pad, v_pad + 2 * v_advance, "Running time: %.*s", running_str->length, running_str->text);
    
    r64 active = TimerElapsedFrom_us(data.time_active);
    string *active_str = TimerString(active);
    RenderString(&overlay_rs, font_index, left, top, h_pad, v_pad + 3 * v_advance, "Active time: %.*s", active_str->length, active_str->text);
    
    RenderString(&overlay_rs, font_index, left, top, h_pad, v_pad + 4 * v_advance, "Delta t: %f", data.dt);
    // ---
    
    // viewport transforms
    matrix4 viewport_matrix = GenerateViewportMatrix(app.window_width, app.window_height);
    
    // perspective division
    td0 = PerspectiveDivision(td0);
    td1 = PerspectiveDivision(td1);
    td2 = PerspectiveDivision(td2);
    
    // rc - resulting coords
    r32 *rc0 = MultMV(viewport_matrix, td0);
    r32 *rc1 = MultMV(viewport_matrix, td1);
    r32 *rc2 = MultMV(viewport_matrix, td2);
    // ---
    
    
    // Write out stuff at the vertex coords
    char *coord_str_format = "%6.2f %6.2f";
    RenderString_centered(&overlay_rs, font_index, rc0[0], app.window_height - rc0[1] - (v_advance / 2.0f), coord_str_format, td0[0], td0[1]);
    RenderString_centered(&overlay_rs, font_index, rc1[0], app.window_height - rc1[1] - (v_advance / 2.0f), coord_str_format, td1[0], td1[1]);
    RenderString_centered(&overlay_rs, font_index, rc2[0], app.window_height - rc2[1] - (v_advance / 2.0f), coord_str_format, td2[0], td2[1]);
    
    RenderString_centered(&overlay_rs, font_index, rc0[0], app.window_height - rc0[1] + (v_advance / 2.0f), coord_str_format, rc0[0], rc0[1]);
    RenderString_centered(&overlay_rs, font_index, rc1[0], app.window_height - rc1[1] + (v_advance / 2.0f), coord_str_format, rc1[0], rc1[1]);
    RenderString_centered(&overlay_rs, font_index, rc2[0], app.window_height - rc2[1] + (v_advance / 2.0f), coord_str_format, rc2[0], rc2[1]);
    
    // write string geometry to buffers
    void *vertex_mapptr;
    vkMapMemory(vk.device, overlay_rs.vertex_memory, 0, VK_WHOLE_SIZE, 0, &vertex_mapptr);
    memcpy(vertex_mapptr, overlay_rs.geometry->start, overlay_rs.geometry->used);
    vkUnmapMemory(vk.device, overlay_rs.vertex_memory);
    
    
    
    // --- Indexes
    // generate
    u32 *overlay_indexes = GenerateIndexes_overlay(&overlay_rs);
    IndexesWrite(&overlay_rs, overlay_indexes);
    
    // write
    void *overlay_indexes_mapptr;
    vkMapMemory(vk.device, overlay_rs.index_memory, 0, VK_WHOLE_SIZE, 0, &overlay_indexes_mapptr);
    memcpy(overlay_indexes_mapptr, overlay_rs.indexes->start, overlay_rs.indexes->used);
    vkUnmapMemory(vk.device, overlay_rs.index_memory);
    
    
    
    // --- Cleanup
    MemoryReset_geometry(&triangle_rs);
...
void Render()
{
    ...
    VkRenderPassBeginInfo geometry_renderpass_bi = {};
    geometry_renderpass_bi.sType           = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
    geometry_renderpass_bi.pNext           = NULL;
    geometry_renderpass_bi.renderPass      = triangle_rs.renderpass;
    geometry_renderpass_bi.framebuffer     = vk.framebuffers[data.acquired_index];
    geometry_renderpass_bi.renderArea      = vk.scissor;
    geometry_renderpass_bi.clearValueCount = 1;
    geometry_renderpass_bi.pClearValues    = &clear_color;
    ...
    VkWriteDescriptorSet matrix_write = {};
    matrix_write.sType            = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    matrix_write.pNext            = NULL;
    matrix_write.dstSet           = data.dset;
    matrix_write.dstBinding       = data.matrix_binding;
    matrix_write.dstArrayElement  = 0;
    matrix_write.descriptorCount  = vk.desc_count;
    matrix_write.descriptorType   = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;  // this should be a parameter somewhere
    matrix_write.pImageInfo       = NULL;
    matrix_write.pBufferInfo      = &matrix_buffer_info;
    matrix_write.pTexelBufferView = NULL;
    
    VkDescriptorImageInfo sampler_image_info = {};
    sampler_image_info.sampler     = overlay_rs.fd->atlas_sampler;
    sampler_image_info.imageView   = overlay_rs.fd->atlas_imageview;
    sampler_image_info.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    
    VkWriteDescriptorSet sampler_write = {};
    sampler_write.sType            = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    sampler_write.pNext            = NULL;
    sampler_write.dstSet           = data.dset;
    sampler_write.dstBinding       = overlay_rs.fd->atlas_sampler_binding;
    sampler_write.dstArrayElement  = 0;
    sampler_write.descriptorCount  = vk.desc_count;
    sampler_write.descriptorType   = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    sampler_write.pImageInfo       = &sampler_image_info;
    sampler_write.pBufferInfo      = NULL;
    sampler_write.pTexelBufferView = NULL;
    
    VkWriteDescriptorSet desc_writes[] = { matrix_write, sampler_write };
    
    vkUpdateDescriptorSets(vk.device, 2, desc_writes, 0, NULL);
    // ---
    ...
    vkCmdBindVertexBuffers(vk.command_buffer, data.vertex_binding, 1, &triangle_rs.vertex_buffer, &offset);
    vkCmdBindIndexBuffer(vk.command_buffer, triangle_rs.index_buffer, 0, VK_INDEX_TYPE_UINT32);
    vkCmdBindPipeline(vk.command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, triangle_rs.pipeline);
    vkCmdBeginRenderPass(vk.command_buffer, &geometry_renderpass_bi, VK_SUBPASS_CONTENTS_INLINE);
    vkCmdDrawIndexed(vk.command_buffer, triangle_rs.index_count, 1, 0, 0, 0);
    ...
    vkCmdBindDescriptorSets(vk.command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, overlay_rs.pipeline_layout, 0, vk.desc_count, &data.dset, 0, NULL);
